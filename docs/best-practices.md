...
## 字幕语言最佳实践（YouTube → 中文文章）
- 优先中文：先判断是否存在 `zh-Hans/zh/zh-Hant` 字幕轨；有则直接下载中文作为下游输入
- 回退英文：若无中文则下载英文（或高质量自动英文），逐 cue 翻译为中文，保持时间码一一对应
- 双层保存：源层与中文层均需保存，命名与对齐规则如下：
  - 源层：`video.src.vtt`、`video.src.srt`（真实下载语言）
  - 中文层：`video.zh.vtt`、`video.zh.srt`（原生中文或翻译中文）
- 目录规范：`~/Downloads/youtube-media/<VIDEO_ID>/subtitles/`
- Phase 0 产物：`phase0_manifest.json`、`translation_report.json`（记录轨道、来源与质量指标）
- Downstream 默认：清洗、分块与写作默认走中文层；不可用时回退源层
